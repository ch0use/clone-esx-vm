...
for VM_DISK in ${VM_DISKS_SOURCE}; do
    SECTORS=$(grep VMFS "${VM_HOMEDIR_SOURCE}/${VM_DISK}" | awk '{print($2)}')
    echo "$(date) Creating disk ${VM_DISK} of size ${SECTORS} sectors using ${VM_CLONE_DISK_FORMAT} format ..."
    # need to add -W vsan if source is on VSAN datastore
    vmkfstools -c ${SECTORS}s "${VM_PATH_CLONE}/${VM_DISK}" -d ${VM_CLONE_DISK_FORMAT}
    if [[ ${?} -gt 0 ]]; then
        echo "$(date) Some error creating ${VM_PATH_CLONE}/${VM_DISK} of size ${SECTORS} sectors using ${VM_CLONE_DISK_FORMAT} format! Continuing ..."
    else
        SCSI_PATH=$(grep ${VM_DISK} ${VM_VMX_PATH_SOURCE} | cut -d "=" -f 1)
        echo "$(date) Disk created successfully."
